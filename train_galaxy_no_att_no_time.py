...
ATTENTION_HEADS = 0        # Deafault =8 / Set to 0 for no attention
TIME_EMB_DIM = 0         # Default=256 / Set to None for no time embedding
...
WANDB_RUN_NAME = f"galaxy_UNet_attention_{ATTENTION_HEADS}_timeemb_{TIME_EMB_DIM}_epochs_{EPOCHS}"  # Descriptive name for this run
...
SAVE_DIR = 'models'
RESULTS_DIR = 'results'
SAVE_PATH = f"{SAVE_DIR}/galaxy_{timestamp}.pkl"
LOSS_PATH = f'{RESULTS_DIR}/galaxy_losses_{timestamp}.pkl'
...
train_set_noisy = NoisyDataset(train_set, scheduler)
test_set_noisy = NoisyDataset(test_set, scheduler)
...
print(f"📁 Loaded from module: {unet.__module__}")
...
print(f"  Time embedding dimension: {TIME_EMB_DIM}")
